...
SHOW_MS   = 3000   # tiempo visible
FADE_STEP = 0.09   # paso de opacidad por frame
FRAME_MS  = 16     # ~60 fps
...
class ShortcutOverlay:
    ...
    def _build(self, info: dict):
        ...
        tk.Label(
            outer, text="⌨  Atajo disponible",
            font=("Segoe UI", 8), fg=TEXT_HINT, bg=DARK_BG,
        ).pack(anchor="w")

        # Shortcut en grande
        tk.Label(
            outer, text=info["shortcut"],
            font=("Segoe UI", 15, "bold"), fg=ACCENT, bg=DARK_BG,
        ).pack(anchor="w", pady=(2, 0))

        # Descripción de la acción
        tk.Label(
            outer, text=info["action"],
            font=("Segoe UI", 9), fg=TEXT_MAIN, bg=DARK_BG,
        ).pack(anchor="w")

        # Nombre de la app (en pequeño)
        tk.Label(
            outer, text=info.get("app", ""),
            font=("Segoe UI", 7), fg=TEXT_SUB, bg=DARK_BG,
    ...
    def _no_activate(popup: tk.Toplevel):
        """Evita que el popup robe el foco usando Win32."""
        try:
